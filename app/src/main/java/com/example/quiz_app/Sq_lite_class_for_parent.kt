...
class Sq_lite_class_for_parent(context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    companion object {
        // If you change the database schema, you must increment the database version.
        const val DATABASE_VERSION = 1
        const val DATABASE_NAME = "Parent_Database"
        const val TABLE_NAME="parent"
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        var query:String="""DROP TABLE IF EXISTS parent"""
        db?.execSQL(query)
    }
    ...
    fun select_new_user(): ArrayList<user_details>
    {
        ...
        var c1=selecting.rawQuery("SELECT Username,Email,Password FROM parent",null)
